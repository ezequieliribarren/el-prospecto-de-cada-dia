  ...
  for (let d = 0; d < daysCount; d++) {
    ...
    for (let a = 0; a < accIds.length; a++) {
      ...
      for (let c = 0; c < perDay; c++) {
        ...
        if (!cand) break;
        await insert.run(cand.id, date, accountLabel, assignedUserId);
      }
    }
  }

  const assigned = (await db.prepare(`SELECT COUNT(*) as c FROM plan`).get()).c;
        ...
        if (backlog.length) {
          for (const d of dates) {
            ...
            if (missing > 0 && backlog.length) {
              ...
              await updateDateByIdStmt(take, d);
            }
          ...
          while (backlog.length) {
            ...
            if (missing > 0) {
              const take = backlog.splice(0, missing);
              await updateDateByIdStmt(take, carryDate);
            } else {
            ...
            if (pendings.length > perDay) {
              ...
              while (remaining.length) {
                ...
                if (missingTgt > 0) {
                  ...
                  await updateDateByIdStmt(take, targetDate);
                } else {
                  j += 1;
                }
...
async function autoGenerate(req, res) {
  const db = getDb();
  const perDay = Math.max(1, Math.min(200, Number(req.body.per_day) || Number(((await db.prepare(`SELECT value FROM settings WHERE key='per_day'`).get())||{value:'25'}).value) || 25));
  const startDate = req.body.start_date || dayjs().format('YYYY-MM-DD');
  const daysCount = Math.max(1, Math.min(365, Number(req.body.days) || 30));
  let activeSenders = [];
  try { activeSenders = JSON.parse(((await db.prepare(`SELECT value FROM settings WHERE key='active_senders'`).get())||{value:'[]'}).value); } catch { activeSenders = []; }
  const senders = activeSenders.length
    ? await db.prepare(`SELECT id, username FROM users WHERE role='sender' AND id IN (${activeSenders.map(()=>'?').join(',')})`).all(...activeSenders)
    : await db.prepare(`SELECT id, username FROM users WHERE role='sender'`).all();
  ...
  const insert = db.prepare(`INSERT INTO plan (prospect_id, date, account_label, assigned_user_id, status) VALUES (?, ?, ?, ?, 'pending')`);
  let idx2 = 0; let d = dayjs(startDate);
  for (let di = 0; di < daysCount; ) {
    ...
    for (const acc of roundAccounts) {
      for (let c=0;c<perDay;c++){
        const cand = candidates[idx2++];
        if (!cand) break;
        await insert.run(cand.id, date, acc.username || `Account`, acc.id || null);
      }
    }
    d = d.add(1,'day');
    di++;
...
async function updatePlanStatus(req, res) {
  ...
  if (req.user?.role === 'sender') {
    const pl = await db.prepare(`SELECT assigned_user_id FROM plan WHERE id=?`).get(id);
    if (!pl) return res.status(404).json({ error: 'Not found' });
    if (pl.assigned_user_id != null && pl.assigned_user_id !== uid) {
      return res.status(403).json({ error: 'Forbidden' });
    }
  }
  const r = await db.prepare(`UPDATE plan SET status=?, updated_by_user_id=? WHERE id=?`).run(status, uid, id);
