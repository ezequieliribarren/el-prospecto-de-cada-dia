...
function getRangePlan(req, res) {
  ...
  const senders = activeSenders.length ? db.prepare(`SELECT id, username FROM users WHERE id IN (${activeSenders.map(()=>'?').join(',')})`).all(...activeSenders) : [];

  if (senders.length) {
    // ensure there are enough plan rows for the requested range
    const countStmt = db.prepare(`SELECT COUNT(*) as c FROM plan WHERE date=? AND assigned_user_id=?`);
    ...
    const tx = db.transaction(() => {
      for (const d of dates) {
        for (const s of senders) {
          const have = countStmt.get(d, s.id).c;
          const missing = Math.max(0, perDay - have);
          if (missing > 0) {
...
function autoGenerate(req, res) {
  ...
  let activeSenders = [];
  try { activeSenders = JSON.parse((db.prepare(`SELECT value FROM settings WHERE key='active_senders'`).get()||{value:'[]'}).value); } catch { activeSenders = []; }
  const senders = activeSenders.length ? db.prepare(`SELECT id, username FROM users WHERE id IN (${activeSenders.map(()=>'?').join(',')})`).all(...activeSenders) : db.prepare(`SELECT id, username FROM users WHERE role='sender'`).all();
...
function updatePlanStatus(req, res) {
  ...
  if (!['pending', 'sent', 'won'].includes(status)) {
