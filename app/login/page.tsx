  ...
  return (
    <div className="max-w-sm mx-auto card p-6 space-y-4">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div>
          <div className="text-sm text-white/70 mb-1">Contraseña</div>
      ...
      {openBootstrap && (
        <form className="space-y-3 pt-2" onSubmit={async (e)=>{e.preventDefault(); setErr(''); try { await api.post('/auth/register', { ...adminForm, role: 'admin' }); alert('Admin creado. Ahora inicia sesión.'); setOpenBootstrap(false);} catch (e:any){ setErr(e?.response?.data?.error || 'No se pudo crear'); } }}>
          <div>
          ...
          <div>
            <div className="text-sm text-white/70 mb-1">Número (Argentina)</div>
            <PhoneInput
              country="AR"
              international
